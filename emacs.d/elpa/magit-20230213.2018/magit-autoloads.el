...
(defun magit-maybe-define-global-key-bindings (&optional force) (when magit-define-global-key-bindings (let ((map (current-global-map))) (pcase-dolist (`(,key \, def) '(("C-x g" . magit-status) ("C-x M-g" . magit-dispatch) ("C-c M-g" . magit-file-dispatch))) (when (or force (not (or (keymap-lookup map key) (where-is-internal def (make-sparse-keymap) t)))) (keymap-set map key def))))))
...
Auto-Revert mode is enabled in all buffers where `magit-turn-on-auto-revert-mode-if-desired' would do
it.
